...
def _find_cutoff(values, neg_values, cutoff_method, cutoff_value, do_plot, plot_folder):
    ...
    if cutoff_method == 'empiric_FDR':
        ...
        p = plt.figure()
        plt.hist(pred_neg, normed=True, range=(0.01, 1), bins=3100, alpha=1, label="sim_ctrl")
        ...
        p = plt.figure()
        plt.hist(values, normed=True, range=(0.01, 1), bins=100, alpha=1, label="Overexpressions")
        plt.hist(neg_values, normed=True, range=(0.01, 1), bins=300, alpha=0.8,
                 label="Gamma of control overexpressions")
        # FDR rate
        fdr = (max(sum(neg_values > vr_tresh), 1) / len(neg_values) * len(values)) / sum(values > vr_tresh)
        p.axes[0].axvline(vr_tresh, color='k', linestyle='--', label='Cutoff: %.2f\nEmpiric FDR: %.2f' % vr_tresh % fdr)
        plt.legend(loc=1);
        if do_plot:
            p.savefig(os.path.join(plot_folder, 'Cutoff' + '.pdf'))
...
def adnet_analysis(parser):
    ...
    """
    print('Start loading')
    complete_data, channels = _load_all_data(base_folder, sub_folders, row_col_fn_pos, sep)

    print('Clean the single cell data')
    complete_data, channels = _clean_data(complete_data, meta_cols_all, channels, crap_markers, name_dict)
    ...
    bin_dat = _calculate_bin_stats(complete_data, meta_cols_all, meta_cols_bin, channel_target, min_cells,
                                   bin_stat, bin_range, nbins)

    print('pearson/spearman calculation started')
    bin_dat = _calculate_correlations(bin_dat, complete_data)
    ...
    bin_dat, vr_tresh  = _find_cutoff_from_table(bin_dat, cutoff_stat, cutoff_method, neg_ctrl_names, cutoff_value, do_plot, plot_folder)
    ...
    def _plot_violins(complete_data, bin_dat, plot_idx, stats, experiment_str='experiment', marker_str='marker',
        ...
        for marker, target, origin in marker_target_origin:
            ...
            for i, idx in enumerate(itertools.product(experiment,timepoint)):
                ...
                if idx in plot_dat.index:
                    ...
                    title =  "BP-R2: %1.2f\nBin Spearman: %1.2f\nPearson R: %1.2f, Spearman R: %1.2f" % (ratio, binspear,pears, spear)
                    p.set_title(title)
                    plt.ylabel(idx[:-1])
                    plt.xlabel(idx[-1])
    ...
    _plot_violins(complete_data, bin_dat, idx, stats, experiment_str='experiment', marker_str='marker',
                  target_str='target', origin_str='origin', timepoint_str='timepoint')
    ...
    if do_plot:
        ...
        for mark in uni_mark:
            ...
            plot_dat[rescaled_name] = plot_dat.groupby(['target','experiment'])[bin_stat].transform(lambda x: x/np.max(x))
            ...
            p.savefig(os.path.join(plot_folder,lib.format_filename( 'Trends_EGF_overexpression_' + mark  + '.png')), figsize=(25,15))
            p.savefig(os.path.join(plot_folder, lib.format_filename('Trends_EGF_overexpression_' + mark  + '.pdf')), figsize=(25,15))
            ...
            p.fig.subplots_adjust(top=.9)
            p.savefig(os.path.join(plot_folder, lib.format_filename('Trends_EGF_overexpression_' + mark  + '_TPoverBins'+'.png')), figsize=(25,15))
            if plot_pdf:
                p.savefig(os.path.join(plot_folder,lib.format_filename( 'Trends_EGF_overexpression_' + mark  + '_TPoverBins'+'.pdf')), figsize=(25,15))
            ...
            if nTP > 1:
                ...
                plt.suptitle('Overexpression: '+ mark)
                p.fig.subplots_adjust(top=.9)
                p.savefig(os.path.join(plot_folder,lib.format_filename( 'Trends_EGF_overexpression_' + mark  + '_BinsoverTP'+'.png')), figsize=(25,15))
                if plot_pdf:
                    p.savefig(os.path.join(plot_folder,lib.format_filename( 'Trends_EGF_overexpression_' + mark  + '_BinsoverTP'+'.pdf')), figsize=(25,15))
                plt.close()
