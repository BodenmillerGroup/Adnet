...
dat_dremi_stacked = pd.merge(t_dat, dat_dremi_stacked, how='outer',on=['experiment', 'row_col', 'origin', 'target'])
dat_dremi_stacked.head()
...
# Filter out the non wanted markers & Prepare the negative control filter
...
plt.savefig(os.path.join(out_folder,'20160429_readout_comparison_venn_wocc_alltp.pdf'))
...
hit_tab.to_csv(os.path.join(out_folder,'20160429_readout_comparison_vsemptygfp_woCC_alltp.csv'),index=False)
...
bin_dat.xs('MAPK1', level='marker').xs('p-ERK1-2', level='target')['stats']['median_mean_var_ratio']


# In[242]:

bin_dat.xs('MAPK1', level='marker').xs('p-ERK1-2', level='target')['stats']['dremi_median']
